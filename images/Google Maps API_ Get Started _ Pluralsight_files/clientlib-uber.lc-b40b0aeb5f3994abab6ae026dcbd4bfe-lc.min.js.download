...
(function ($) {
  ...
  $coursePages.each(function () {
    ...
    function setStickies() {
      ...
      let navHeight = jQuery('nav.ps-nav-global').height() + jQuery('nav.ps-nav-global').position().top;
...
jQuery(function() {
    var $badgePageComponents = jQuery('#content .mtvl-badges');

    $badgePageComponents.each(function() {
        var $badgePageComponent = jQuery(this);
        var needsRedirect = $badgePageComponent.data( '404-redirect' );

        if (needsRedirect) {
            window.location = "https://www.pluralsight.com/404";
        }
    });
});
...
if (jQuery("#search-field form").length) {
        ...
        if (CludoSearch) {
            if (category === "all") {
                ...
                const allFacets = [
                    ...
                    {
                        "all":true,
                        "category":["paths","course","labs","hands-on learning","blog","resource","aem-author","event","page"],
                        "perPage":"1",
                        "complete":false
                    }];
    ...
    function getSearchParamsFromURL() {
        ...
        function stringIsFacetName(string) {
            const FACET_PARAM_NAMES = ["roles", "skillLevels", "contentType", "postTags", "eventType","eventRegion"];
...
if (jQuery("#search-filter-bar").length) {
    ...
    $filterBar.on('click', '#search-filter-tabs a', function(e, pushHistoryState) {
        ...
        changeHistoryQueryParam("categories",
            jQuery(this).attr("data-categories"), false, false);
        window.searchFromURL();
    });
    ...
    function clearFacetsSortParam(){
        ...
        if(qParamsEncoded !== "?"){
            var FACET_PARAM_NAMES = ["roles", "skillLevels", "contentType", "postTags","eventType","eventRegion"];
    ...
    function updateFilterBarUI(eventData, category, facets, sort, loadNextPage) {
        ...
        if ($mobileSearchTabsDropdown.length) {
            $mobileSearchTabsDropdown.val(category);
        }
...
(() => {
    ...
    const init = () => {
        ...
        const toggleUI = jQuery("#toggle");
        const grid = jQuery("#grid");
        if (pricingComponent.length > 0 && !!toggleUI && !!grid) {
            toggleUI.on("change", (e) => {
                if (toggleUI.is(":checked")) {
                    grid.removeClass("show-monthly");
                    toggleMiniParents.removeClass("show-monthly")
                } else {
                    grid.addClass("show-monthly");
                    toggleMiniParents.removeClass("show-monthly")
                }
            });
        }
        ...
        const toggleMiniParents = jQuery(".pricing_section");
        let isResizeDone;
        ...
        toggleMini.off("change.psPricingToggleMini");
        toggleMini.on("change.psPricingToggleMini", function () {
            const cardParent = jQuery(this).parents(".pricing_section");
            jQuery(grid).removeClass("show-monthly")
...
document.addEventListener("DOMContentLoaded", function(event) {
    var allLinkSelector = 'a[href*="pluralsight.com/individual/checkout/account-details"], a[href*="pluralsight.com/checkout"], a[href*="pluralsight.com/offer-landing"], a[href*="pluralsight.com/gift-of-pluralsight"][href*="billing"], a[href*="pluralsight.com/buy"]';
    ...
    var setProductLinks = function(productApiData, callback) {
        ...
        if (document.URL.indexOf("-stage.pluralsight") > -1 || document.URL.indexOf("aem-local-author.pluralsight") > -1){
                    ...
                    [].forEach.call(offering.options, function(option) {
                        if (option.marketingId === marketingId) {
                            var swapLinkEls = function(pricingOption) {
                                ...
                                if (linkElHref.indexOf("pluralsight.com/checkout") > -1
                                || (linkElHref.indexOf("pluralsight.com/buy") > -1 && linkElHref.indexOf("PROFESSIONAL-SG") > -1)) {
                                    if (!linkEl.classList.contains("biz_qty_checkout") && linkElHref.indexOf("nomodal=true") == -1){
    ...
    try{
        ...
        if(window.location.pathname === '/pricing/skills'
            || window.location.pathname === '/pricing/free-trial'
            || window.location.pathname === '/product/skills/personal'
            || window.location.pathname === '/pricing/redirect-to-checkout') {
            targetPricingOff = false;
            //Temporary add this for our developing countries price change after target loads.
        }
        setTimeout(function(){ if (!success){dtmEventTrigger("ps.pricingServiceTimeout",{});} }, 6000);
        ...
        if (document.querySelectorAll('[data-product-price]').length || document.querySelectorAll(allLinkSelector).length || document.querySelectorAll('.targeted-location').length) {
            ...
            try {
                ...
                getJSON(PRODUCT_URL, function(err, data) {
                    if (err !== null) {
                        console.log('Pricing catalog JSON failed: ' + err);
                ...
                console.log('Placing price data from catalog failed: ' + err);
                dtmEventTrigger("ps.pricingServiceError",{});
            }
        } else {
        ...
        console.log('Pricing catalog parent function failed: ' + err);
        dtmEventTrigger("ps.pricingServiceError",{});
    }
});
    ...
    $proQtyModals.each(function () {
      ...
      jQuery(document).on('click.psProQtyModal', 'a[href="#ps_pro_qty_modal"]', function (e) {
        ...
        if (jQuery(this).data('product-sku') === "PROFESSIONAL-SG-PILOT" || jQuery(this).data('product-sku') === "PRO-SG-PILOT-AC") {
          ...
          quantity = 10; //reset this in case they viewed purchase first.
        }
        $proQtyModal.show();
        setVisibleQuantity();
        window.setTimeout(function () {
      ...
      $proQtyDown.add($proQtyUp).on('click.psProQtyModal', function (e) {
        ...
        if (jQuery(this).hasClass("qty-down")) {
          ...
          if ((!isPilotModal && quantity < 50) || quantity < 10) {
            quantity = quantity + 1;
          }
        }
        setVisibleQuantity();
...
(function () {
  ...
  const init = () => {
    ...
    let munchkinIdSetting = "306-DUP-745";
    ...
    const formSetup = () => {
      jQuery(".marketo-form-component").each(function (key, component) {
        ...
        if (!thisForm.$form.hasClass('loaded')) {
          ...
          thisForm.formPreset = thisForm.$marketoFormData.find(".form-preset").html().trim();
          ...
          thisForm.formID = thisForm.formPreset || thisForm.authoredFormID;
          ...
          const embedForm = function (currentForm) {
            MktoForms2.loadForm(marketoUrlSetting, munchkinIdSetting, currentForm.formID, function (form) {
              ...
              const mcVals = Cookies.get('AMCV_70D658CC558978FF7F000101@AdobeOrg');
              if (mcVals) {
              ...
              form.onSubmit(function (form) {
                // TODO: commenting out DNB field?
                // if (currentForm.$form.find('input[name="VIFF_DB_DUNS__c"]').length > 0) {
                //     form.vals({"DunsNumber__c": currentForm.$form.find('input[name="VIFF_DB_DUNS__c"]').val()});
                // }
                ...
                // TODO: what are these for, they appear to be unused ... commenting out for now, but proposing complete removal
                // const oidValue = 'oid=' + jQuery('meta#view-oid').data('view-oid');
                // const widValue = 'wid=' + jQuery('.marketo-wid').text();
              });
              form.onSuccess(function (values, followUpUrl) {
                ...
                if(Cookies.get("ps_optout") == 1) { // if they opted out, don't track?
                  Cookies.set("marketoFilled", 1, { expires: 7, path: '/' });
                }
  ...
  $browseSearchComponents.each(function () {
    ...
    $facets.each(function() {
      ...
      if(facetDefault) {
        ...
        selectedFacets[key] = facetDefault.split(',').map(function(s) { return s.trim(); });
      } else {
        selectedFacets[key] = [];
      }
    });
    ...
    const getCludoResults = (pushState) => {
      ...
      // sort default query by best rated and new
      if (searchTerm === '*') {
        requestBody.sort = {'rating':'desc','display-date-formatted':'desc'};
      }

      if (sortBy === 'default' || sortByNative === 'default') {
        requestBody.sort = {'rating':'desc','display-date-formatted':'desc'};
      } else {
        hasFilters = true;
        ...
        searchResultsHtml += `<li class="browse-search-results-item large-12 columns">
                                <a href="${url}" class="browse-search-results-item-link" data-aa-title="browse-search-result">
                                  <div class="course-details">
                                    ...
                                    <h4>by ${author}</h4>
                                    ${isStandardLibrary === 'false' ? '<div class="not-standard-label"  data-tooltip="Access to our full 7,000+ course library, including more advanced content."><strong>Expanded</strong></div>' : ''}
                                    ${isLabs === 'labs' ? '<div class="is-labs-label"><img src="/etc.clientlibs/pluralsight/clientlibs/clientlib-main/resources/images/labs/icon-labs-white.png" alt="Labs"/>Lab</div>' : ''}
                                    <br class="rwd-break">
                                    <div class="duration">${parsedDuration} <img src="/etc.clientlibs/pluralsight/clientlibs/clientlib-main/resources/images/course/clock.png" class="course-info__row--icon"></div>
                                    <br class="rwd-break">
    ...
    const routeToState = (force) => {
      ...
      } else if(force) { $searchForm.find('input.browse-search-form-text').val(''); }
      ...
      $facets.each(function() {
        ...
        let val = query[param];
        if(val) {
          selectedFacets[key] = val.split('|').map(function(s) { return s.trim(); });
          doCheck();
        } else if (force) {
          selectedFacets[key] = [];
        }
    ...
    const stateToRoute = () => {
      ...
      if(q) { query.q = q; changed = true; }
      else { delete query.q; changed = true; }

      let filter = getCurrentFilter();
      if(filter) { query.type = filter; changed = true; }
      else { delete query.type; changed = true; }

      let sort = getCurrentSortBy();
      if(sort) { query.sort = sort; changed = true; }
      else { delete query.sort; changed = true; }
      ...
      $facets.each(function() {
        ...
        let val = facet.join('|');
        if(val) { query[param] = val; changed = true; }
        else { delete query[param]; changed = true; }
      });
...
if(jQuery(".generic-block-core").length){
    ...
    if(jQuery(".generic-block-core .block--sticky").length){
        ...
        window.setStickies = function(){
            var navHeight = 0;
            if (jQuery('nav.ps-nav-global').length){
                navHeight = jQuery('nav.ps-nav-global').height() + jQuery('nav.ps-nav-global').position().top;
            }
            jQuery.each( jQuery('.generic-block-wrapper.block--sticky'), function( key, value ) {
                ...
                var $header = jQuery("nav.ps-nav-global");
